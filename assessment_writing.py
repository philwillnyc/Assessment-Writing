"""Contains the main functions for processing question banks and outputting latex documents."""
...
def indices(open,string):
    """Given a substring of opening characters and a string,
     returns a list of indices for where the opening characters occur."""
    i = string.find(open)
    k = len(open)
    n = 0
    L = []
...
def read(string, open, close):
    """Extract the string between the first instances of the opening characters and the close characters.
    Returns a pair: the string alone, and also the string with the opening and closing characters included."""
...
def replacement(string, open, close, variables):
    """Evaluate the python expressions between each set of open and close characters returns the modified text.
    The variables parameter includes context supplied by the output of another evaluation, defined below."""
    new_string = string
